...
function addSeatedLumbarControlMetric(
  ...
  if (isLandmarkVisible(LANDMARKS.LEFT_SHOULDER) && 
    ...
    // 座位膝伸展テスト用の評価基準（6.5°までは緩やか、それ以降は厳しく）
    let controlScore = 0;
    if (excessiveMovement <= 3) {
      controlScore = 100; // 優秀な制御（範囲を拡大）
    } else if (excessiveMovement <= 6.5) {
      controlScore = 100 - ((excessiveMovement - 3) * 3); // 3°超えで3点ずつ減点（緩やか）
    } else if (excessiveMovement <= 8) {
      controlScore = Math.max(0, 89.5 - ((excessiveMovement - 6.5) * 25)); // 6.5°超えで25点ずつ減点（非常に厳しく）
    } else if (excessiveMovement <= 10) {
      controlScore = Math.max(0, 52 - ((excessiveMovement - 8) * 20)); // 8°超えで20点ずつ減点
    } else {
      controlScore = Math.max(0, 12 - ((excessiveMovement - 10) * 2)); // 10°超えで2点ずつ減点
    }
